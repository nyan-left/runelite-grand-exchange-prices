...
import { TransactionData, FullMap, MapData, TimeSeriesData } from "./spec";
 ...
 */
export const latest = async (options: { id?: number; useragent: string }): Promise<TransactionData | TransactionData[]> => {
 ...
 * @see https://oldschool.runescape.wiki/w/RuneScape:Real-time_Prices
 */
export const mapping = async (options: { id?: number; useragent: string }): Promise<FullMap | MapData> => {
  const { id, useragent } = options || {};
